...
function Savings2({ user, setUser, thememode, toggle }) {
  ...
  const handlesavedit = (e, obj) => {
    ...
    const editSavings = async () => {
      try {
        ...
        const res = await axios.put(`${process.env.REACT_APP_BASE_URL}/api/savings/editSaving/${obj._id}`, { sav });
  ...
  useEffect(() => {
    ...
    const getSavings = async () => {
      try {
        const res = await axios.get(` ${process.env.REACT_APP_BASE_URL}/api/savings/getSavings/${user._id}`)//add user Id
  ...
  const handleDelete = () => {
    const delsaving = async () => {
      try {
        const res = await axios.delete(`${process.env.REACT_APP_BASE_URL}/api/savings/deleteSaving/${selectedSavingId}`)
        console.log(res.data)
        setUpdateFlag((prev) => !(prev))
