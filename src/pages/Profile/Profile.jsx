...
function Profile({ user, thememode, toggle, setUser }) {
      ...
      .then(() => {
        getDownloadURL(imageRef)
          .then((url) => {
            const addUrl = async () => {
              try {
                const res = await axios.put(`${process.env.REACT_APP_BASE_URL}/api/user/addImg/${user._id}`, { url })
  ...
  React.useEffect(() => {
    const getBadges = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/api/user/getBadges/${user._id}`);
