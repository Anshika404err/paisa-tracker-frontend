...
const Dashboard = ({ user, thememode, toggle, setUser }) => {
  ...
  useEffect(() => {
    ...
    const ifnocategselect = async () => {
      try {
        const res = await axios.get(`${process.env.REACT_APP_BASE_URL}/api/transactions/getTransactions/${user._id}`);
  ...
  const handleFilter = e => {
    ...
    const addFilter = async () => {
      try {
        const res = await axios.post(`${process.env.REACT_APP_BASE_URL}/api/transactions/getTransactionsByFilter`, { filterInput })
  ...
  const mailsend = async () => {
    try {
      ...
      console.log(reqmail)
      const res = await axios.post(`${process.env.REACT_APP_BASE_URL}/api/mail/sendmail`, { reqmail })
  ...
  const addBadge = async (img) => {
    try {
      console.log(img)
      const res = await axios.post(`${process.env.REACT_APP_BASE_URL}/api/user/addbadge/${user._id}`, { img })
  ...
  useEffect(() => {
    ...
    const getTotalStats = async () => {
      try {
        ...
        console.log(result);
        const res = await axios.get(` ${process.env.react_app_base_url}l/api/transactions/getTotalStats/${user._id}`)
        console.log(res.data)
        setStats(res.data)
      } catch (err) {
        console.log(err)
  ...
  useEffect(() => {
    const getTrans = async () => {
      try {
        const res = await axios.get(` ${process.env.react_app_base_url}l/api/transactions/getTransactions/${user._id}`)//add user Id
        console.log(res.data)
  ...
  const handleSubmit = async (e) => {
    ...
    const addTrans = async () => {
      try {
        ...
        const res = await axios.post(`${process.env.REACT_APP_BASE_URL}/api/transactions/addTransaction`, { userId: user._id, type, category, desc, date, currency, amount })
        console.log(res.data)
