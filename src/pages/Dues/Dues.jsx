...
function Dues({ user, thememode, toggle, setUser }) {
  ...
  const mailsendstart = async () => {
    try {
      ...
      console.log(reqmail);
      const res = await axios.post(`${process.env.REACT_APP_BASE_URL}/api/mail/sendstartmail`, { reqmail });
  ...
  const mailsendrecurring = async (recurring) => {
    try {
      ...
      console.log(reqmail);
      const res = await axios.post(`${process.env.REACT_APP_BASE_URL}/api/mail/sendmailrecurring`, { reqmail, duedate, recurring });
      alert('Message Sent Successfully');
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    dueItem.amount = Math.floor(dueItem.amount / conversionRate);
    ...
    try {
      const res = await axios.post(`${process.env.REACT_APP_BASE_URL}/api/bills/addBill`, { dueItem });
  ...
  const handleSubmitBill = (e, obj) => {
    ...
    const editBill = async () => {
      try {
        ...
        const res = await axios.put(`${process.env.REACT_APP_BASE_URL}/api/bills/editBill/${obj._id}`, { Bill });
        console.log(res.data);
        setBill({
          userId: user._id,
          titleedit: '',
  ...
  const handleDelete = () => {
    const delBill = async () => {
      try {
        ...
        const res = await axios.delete(`${process.env.REACT_APP_BASE_URL}/api/bills/deleteBill/${selecteddue}`);
        console.log(res.data);
        setbillflag((prev) => !(prev))
